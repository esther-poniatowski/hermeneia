...
def test_profile_resolution_rejects_legacy_math_abstract_framing_rule_id(
    ...
    config = parse_project_config({"rules": {"active": ["math.abstract_framing"]}})
    with pytest.raises(
        ValueError,
...
def test_profile_resolution_rejects_rule_not_supported_by_language_pack(
    ...
    config = parse_project_config({"rules": {"active": ["syntax.passive_voice"]}})
    with pytest.raises(
        ValueError,
...
def test_profile_resolution_accepts_global_block_kind_gating_with_options_model(
    registry, language_pack
) -> None:
    registry.add(_OptionsValidatedRule)
    config = parse_project_config(
    ...
    profile = ProfileResolver(registry).resolve(config, language_pack)
    options = profile.rules["test.options_model"].options
    assert options["max_words"] == 7
    assert options["apply_block_kinds"] == ("paragraph", "table_cell")
    assert options["exclude_block_kinds"] == ("heading",)


def test_profile_resolution_rejects_unknown_global_block_kind_option(
    registry, language_pack
) -> None:
    config = parse_project_config(
        {
            "rules": {
                "overrides": {
                    "syntax.sentence_length": {
                        "options": {"apply_block_kinds": ["unknown_kind"]},
                    }
                }
            }
        }
    ...
    with pytest.raises(
        ValueError,
        match=(
            "Invalid options for rule 'syntax.sentence_length': "
            "apply_block_kinds includes unknown block kind 'unknown_kind'"
        ),
    ):
        ProfileResolver(registry).resolve(config, language_pack)
