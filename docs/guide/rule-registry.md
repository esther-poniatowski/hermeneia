...
| `syntax.multi_action_sentence` | One-action sentence focus | Sentences that stack multiple core actions reduce emphasis and blur argumentative progression. | "The method identifies the boundary and measures transfer." | Split into one primary action per sentence |
...
| `vocabulary.filler_noun_scaffolding` | Filler-noun avoidance | Generic fillers such as *thing* or *aspect* hide the actual technical object. | "Several aspects improve." | "Boundary selectivity and transfer accuracy improve." |
| `vocabulary.cardinality_framing` | Cardinality-first framing control | Taxonomy labels should foreground content, not rhetorical counts. | "Three levels structure the argument." | "The levels structure the argument." |
...
| `reference.structural_metalanguage` | Structural-metalanguage ban | Structural container references (section/table/file scaffolding) should be replaced with content labels. | "The section below summarizes ..." | "The observable summary below shows ..." |
| `reference.citation_as_agent` | Citation-as-agent ban | Citations should support claims, not act as grammatical subjects or containers (style matching is configurable: key/year, numeric, author-year, or custom regex). | "[bernardi2020] shows ..." / "from [bernardi2020]" | "The decoder transfers across contexts [bernardi2020]." |
| `reference.citation_tail_parenthetical` | Tail citation placement | Citation tags should appear at the end of the supported statement (style matching is configurable). | "In [bernardi2020], transfer appears." | "Transfer appears across contexts [bernardi2020]." |
...
| `linkage.transition_quality` | Transition articulation | Flags weakly linked adjacent shifts, if-consequence sentences missing `then`, implicit contrasts without explicit contrast markers, and paragraph chains with low continuity/no connectors. | "If X, Y." / "Sentence A. Sentence B flips polarity." | "If X, then Y." / "Sentence A. By contrast, Sentence B ..." |
...
| `paragraph.inline_case_split` | Inline case split control | Case distinctions packed into one sentence reduce comparability and increase parsing load, including `if/when ...; otherwise ...` and repeated comma-delimited conditionals. | "When X ...; when Y ..." / "If X, A, if Y, B." | Lead-in sentence + explicit case bullets |
...
| `structure.declarative_heading` | Declarative heading form | Headings should state deliverables, not ask questions or issue imperatives. | "Why does transfer emerge?" / "Define transfer geometry" | "Transfer emergence mechanism" |
